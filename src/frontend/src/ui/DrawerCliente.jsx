...
export default function DrawerCliente({clienteId,onClose}){
  ...
  return <><div className="drawer-backdrop" onClick={onClose}/><aside className="drawer">
    <div className="drawer-head"><div><div className="eyebrow">Dossiê de risco</div><div className="drawer-title">{dossie?.cliente?.nome ?? 'Carregando…'}</div><div className="drawer-sub">{dossie?.cliente ? `${dossie.cliente.situacao.replaceAll('_',' ')} · cliente estratégico` : 'Consultando motor de decisão'}</div></div><button className="icon-btn" onClick={onClose}>×</button></div>
