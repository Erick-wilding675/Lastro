...
> **PIVÔ DE DOMÍNIO EM 11/09 (histórico).** O case oficial foi revelado no
> hackathon: problema real da **KRILLTECH** (agtech brasileira nascida de
> parceria com UnB e EMBRAPA, produto Arbolin Biogenesis) — aumento de
> inadimplência, pedidos repentinos de Recuperação Judicial e recuperação de
> capital lenta. O projeto deixou de ser sobre formação de squads ("Talent
> Graph") e passou a ser sobre **risco relacional de crédito e recuperação
> de recebíveis no agro**: o Lastro. Toda a documentação de produto em
> `.ai/docs/` foi revisada em 13/09 para refletir o Lastro como implementado
> — se ainda encontrar referência a "Talent Graph", "squad", "Project
> Genome" ou "Rizoma" (codinome intermediário) fora de `.ai/playbooks/` e
> `.ai/workflows/` (histórico do processo de kickoff, deixado como registro),
> é resíduo remanescente: sinalize.

> **Status do projeto:** o hackathon (PMI-DF 2026, 11–12/09) já terminou. O
> repositório está sendo preparado como peça de portfólio público — os
> prazos e o task tracker abaixo são registro histórico do sprint, não
> trabalho em aberto.
...
| Motor de exposição e scoring (contágio, haircut, agentes) | [docs/matching-model.md](docs/matching-model.md) |
